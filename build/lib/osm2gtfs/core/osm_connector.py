...
class OsmConnector(object):
    ...
    def export_data(self, csv_out):

        import csv

        stops_all=self.stops['regular']
        assert len(self.stops['stations']) == 0
        data_out = []

        for route_id, route in self.routes.items():
            for iter in route.get_itineraries():
                iter_dict = {}
                stop_list = [stops_all[stop_id] for stop_id in iter.get_stops()]
                iter_dict['ref'] = route_id
                iter_dict['fr'] = iter.fr.encode('utf-8')
                iter_dict['to'] = iter.to.encode('utf-8')
                try:
                    iter_dict['dur'] = iter.tags['travel_time'].encode('utf-8')
                except:
                    iter_dict['dur'] = None
                try:
                    iter_dict['freq'] = route.tags['frequency'].encode('utf-8')
                except:
                    iter_dict['freq'] = None
                data_out.append(iter_dict)

        with open(csv_out, 'w') as outfile:
            csv_writer = csv.DictWriter(outfile, fieldnames=data_out[0].keys())
            csv_writer.writeheader()
            csv_writer.writerows(data_out)
        print("Data exported to {}".format(csv_out))
